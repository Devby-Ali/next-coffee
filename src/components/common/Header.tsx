    ...
    <section className="home relative h-50 xs:h-auto mt-14 md:mt-0 xs:aspect-[2/1] md:aspect-auto bg-[url(../../public/images/headerBgMobile.webp)] md:bg-[url(../../public/images/headerBgDesktop.webp)] bg-no-repeat bg-cover bg-[center_top]">
      <div
        className={`container 2xl:px-28 relative h-full ${
          isHome ? 'md:min-h-screen' : 'md:h-100'
        } flex justify-end items-center overflow-y-hidden`}
      >
        <div className={`text-white ${!isHome && 'md:pt-30'}`}>
          <h2
            className={`font-morabba font-bold mb-0.5 md:mb-2 text-2xl ${
              isHome ? 'md:text-6xl/[62px]' : 'md:text-4xl'
            }`}
          >
            قهوه عربیکا تانزانیا
          </h2>
          <span
            className={`font-morabba font-light text-xl ${
              isHome ? 'md:text-5xl/[64px]' : 'md:text-3xl'
            }`}
          >
            یک فنجان بالانس!
          </span>
          <span
            className={`block w-25 h-px md:h-0.5 bg-orange-300 my-3 ${isHome ? 'md:my-8' : ''}`}
          ></span>
          <p
            className={`max-w-[201px] md:max-w-115 text-xs ${
              isHome ? 'md:text-2xl' : 'md:text-xl'
            }`}
          >
